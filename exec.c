...
#include "elf.h"
  ...
  /*
    ...
    栈占用了单独 的一页内存，其中存有 exec 创建的初始数据。
    栈的最上方放着字符串形式的命令行参数以及指向这些参数的指针数组，
    其 下方放的值使得一个程序可以从 main 开始，仿佛刚刚调用了函数 
    main(argc, argv) 。为了防止栈使用了它不应该使用的 页，栈的下方
    有一个保护页。保护页没有映射，因此当栈的增长超出其所在页时就会
    产生异常，因为无法翻译这个错误的地址。
  */
  /*
    exec 是创建地址空间中用户部分的系统调用。
    它根据文件系统中保存的某个文件来初始化用户部分。 
    exec 通过 namei 打开二进制文件。
    然后，它读取 ELF 头。xv6 应用程序以通行的 ELF 格式来 描述，
    该格式在 elf.h 中定义。一个 ELF 二进制文件包括了一个 ELF 头，
    即结构体 struct elfhdr ，然后是连续 几个程序段的头，
    即结构体 struct proghdr 。
    每个 proghdr 都描述了需要载入到内存中的程序段。
    xv6 中的程序只有一个程序段的头，但其他操作系统中可能有多个。
    exec第一步是检查文件是否包含ELF二进制代码。
    exec 通过setupkvm 分配了一个没有用户部分映射的页表。
    再通过allocuvm 为每个ELF段分配内存，然后通过loaduvm 把段的内容载入内存中。
...
exec(char *path, char **argv)
{
  ...
  */
  ...
  /*
    在创建新的内存映像时，如果 exec 发现了错误，
    比如一个无效的程序段，它就会跳转到标记 bad 处，
    释放这段内存映像， 然后返回 -1
  */
  /*
