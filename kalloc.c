...
struct run {
  ...
  /*
    物理内存分配器的结构，其中包含的数据结构就是一个有可分配物理内存页构成的空闲链表。
    分配器使用结构体run, 还用了一个spin lock 来保护空闲链表。 
...
// initializing the allocator; see kinit above.)
  ...
  /*
    kfree 函数 首先将释放内存的每一字节设为1.这使得访问已被释放内存的代码所读到的不是
    原有数据，而是垃圾数据。 这样做的目的是让这种错误的代码尽早崩溃。
    接下来kfree 把v转换成一个指向结构体struct run 的指针，在r-> next 中保留原有空闲列
    表的表头，然后将当前空闲链表设置为r(说白了就是将r加到空闲列表上)。
    xv6中有一个物理内存分配器的结构，其中包含的数据结构就是一个有可分配物理内存页构成
    的空闲链表。分配器使用结构体run, 还用了一个spin lock 来保护空闲链表。 kfree将内存
    加入到的空闲链表就是分配器。
  */
void
kfree(char *v)
